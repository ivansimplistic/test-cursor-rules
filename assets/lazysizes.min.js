...
//lazyload videos, optionally autoplay muted videos
document.addEventListener('lazybeforeunveil', (e)=>{
  if (e.target.tagName == 'VIDEO') {
    const v = e.target;
    if (v.dataset.poster) {
      const size = Math.ceil(window.devicePixelRatio * (v.offsetWidth||window.innerWidth) / 100) * 100;
      const posterUrl = v.dataset.poster.replace('width=4100', 'width=' + size);
      v.setAttribute('poster', posterUrl);
    }
    v.setAttribute('preload', 'auto');
    if (v.dataset.autoplay != undefined && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
